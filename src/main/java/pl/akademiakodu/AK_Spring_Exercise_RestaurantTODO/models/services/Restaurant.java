...
public class Restaurant {
    ...
    public int calculateOrderPrice(OrderR orderR){
        int sum = 0;
//        return orderR.getMealList().stream().mapToInt(m->m.getPrice()).sum();

        for(Meal i : orderR.getMealList())
            sum+=i.getPrice();
...
//        return menu.stream().filter(meal -> meal.getName().equals(mealName)).findFirst().get();
        for(Meal i : menu)
            if (i.getName().equals(mealName)) return i;
        return null;
